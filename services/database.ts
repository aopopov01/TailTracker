...
export interface StoredPetProfile extends Omit<Pet, 'id'> {
  // SQLite-specific fields (number ID instead of string)
...
class DatabaseService {
  ...
  async initialize() {
    ...
    try {
      ...
      // Create pets table with user relationship (supports anonymous users)
      db.execSync(`
        CREATE TABLE IF NOT EXISTS pets (
          ...
          CHECK ((userId IS NOT NULL) OR (anonymous_session_id IS NOT NULL))
        );
      `);
      ...
      try {
        ...
        if (!createdAtColumn) {
          ...
          db.runSync(`UPDATE pets SET createdAt = ? WHERE createdAt IS NULL;`, [
            currentTimestamp,
          ]);
        }
      } catch (columnError) {
      ...
      // Migration: Add anonymous_session_id column and make userId nullable for anonymous users
      try {
        const tableInfo = db.getAllSync('PRAGMA table_info(pets)') as any[];
        const userIdColumn = tableInfo.find(col => col.name === 'userId');
        const anonymousSessionIdColumn = tableInfo.find(
          col => col.name === 'anonymous_session_id'
        );

        // Check if we need to migrate to support anonymous users
        if (
          (userIdColumn && userIdColumn.notnull === 1) ||
          !anonymousSessionIdColumn
        ) {
          console.log('Migrating pets table to support anonymous users...');

          // Create new table with anonymous user support
          db.execSync(`
            CREATE TABLE pets_anonymous_migration (
              id INTEGER PRIMARY KEY AUTOINCREMENT,
              userId TEXT,
              anonymous_session_id TEXT,
              name TEXT,
          ...
          // Copy existing data, preserving userId where it exists
          db.execSync(`
            INSERT INTO pets_anonymous_migration SELECT
              id,
              userId,
              NULL as anonymous_session_id,
              name, species, photos, breed, dateOfBirth, approximateAge, useApproximateAge,
              gender, colorMarkings, weight, weightUnit, height, heightUnit,
              registrationNumber, insuranceProvider, insurancePolicyNumber,
              medicalConditions, medications, allergies, emergencyContact,
          ...
          db.execSync('DROP TABLE pets;');
          db.execSync('ALTER TABLE pets_anonymous_migration RENAME TO pets;');
          ...
          db.execSync(
            'CREATE INDEX IF NOT EXISTS idx_pets_anonymous_session_id ON pets (anonymous_session_id);'
          );

          console.log('✅ Anonymous user migration completed successfully');
        }
      } catch (anonymousMigrationError) {
        console.warn(
          'Anonymous user migration warning (non-critical):',
          anonymousMigrationError
        );
      }
  ...
  async getUserByEmail(
    email: string
  ): Promise<(User & UserCredentials) | null> {
    await this.initialize();

    try {
      const result = db.getFirstSync('SELECT * FROM users WHERE email = ?', [
        email,
      ]) as any;
  ...
  async savePetProfile(profile: Pet, userId: string): Promise<number> {
    await this.initialize();
    ...
    try {
      ...
      // Handle anonymous users - ensure CHECK constraint is satisfied
      const isAnonymous =
        !userId || userId === 'anonymous_user' || userId === '';
      const actualUserId = isAnonymous ? null : userId;
      const anonymousSessionId = isAnonymous
        ? `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
        : null;

      // Debug logging to understand the constraint failure
      console.log(
        'savePetProfile - userId:',
        userId,
        'isAnonymous:',
        isAnonymous,
        'actualUserId:',
      ...
      const result = db.runSync(
        `INSERT INTO pets (
          userId, anonymous_session_id, name, species, photos, breed, dateOfBirth, approximateAge, useApproximateAge,
          gender, colorMarkings, weight, weightUnit, height, heightUnit,
          registrationNumber, insuranceProvider, insurancePolicyNumber,
          medicalConditions, medications, allergies, emergencyContact,
          personalityTraits, favoriteToys, favoriteActivities, exerciseNeeds,
  ...
  async getAllPets(userId: string): Promise<StoredPetProfile[]> {
    ...
    try {
      ...
      if (userId === 'anonymous_user') {
        // For anonymous users, get all pets with anonymous_session_id
        results = db.getAllSync(
          'SELECT * FROM pets WHERE anonymous_session_id IS NOT NULL ORDER BY createdAt DESC'
        ) as any[];
      } else {
        // For authenticated users, get pets by userId
        results = db.getAllSync(
  ...
  async getUserPets(userId: string): Promise<StoredPetProfile[]> {
    return this.getAllPets(userId);
  }

  async getPetById(
    id: number,
    userId: string
  ): Promise<StoredPetProfile | null> {
    await this.initialize();
    ...
    try {
      ...
      if (userId === 'anonymous_user') {
        // For anonymous users, get pet by id with anonymous_session_id
        result = db.getFirstSync(
          'SELECT * FROM pets WHERE id = ? AND anonymous_session_id IS NOT NULL',
          [id]
        ) as any;
      } else {
        // For authenticated users, get pet by id and userId
        result = db.getFirstSync(
  ...
  async updatePetProfile(
    ...
    try {
      ...
      if (userId === 'anonymous_user') {
        // For anonymous users, update by id with anonymous_session_id
        db.runSync(
          `UPDATE pets SET ${setClause}, updatedAt = ? WHERE id = ? AND anonymous_session_id IS NOT NULL`,
          [...values, now, id] as (string | number)[]
        );
      } else {
        // For authenticated users, update by id and userId
        db.runSync(
  ...
  async deletePet(id: number, userId: string): Promise<void> {
    await this.initialize();

    try {
      if (userId === 'anonymous_user') {
        // For anonymous users, delete by id with anonymous_session_id
        db.runSync(
          'DELETE FROM pets WHERE id = ? AND anonymous_session_id IS NOT NULL',
          [id]
        );
      } else {
        // For authenticated users, delete by id and userId
        db.runSync('DELETE FROM pets WHERE id = ? AND userId = ?', [
          id,
          userId,
        ]);
      }
  ...
  async revokeSharingToken(
    ...
    try {
      ...
      db.runSync(`UPDATE shared_access SET isActive = 0 WHERE tokenId = ?`, [
        tokenId,
      ]);
    } catch (_error) {
      throw _error;
  ...
  async updateVaccinationRecord(
    ...
    try {
      ...
      const values = fields.map(
        field => updates[field as keyof VaccinationRecord]
      );
      const setClause = fields.map(field => `${field} = ?`).join(', ');
  ...
  async updateMedicalRecord(
    ...
    try {
      ...
      const values = fields.map(field => {
        ...
        return field === 'attachments' && Array.isArray(value)
  ...
  async createLostPetAlert(
    alert: Omit<LostPetAlert, 'id' | 'created_at' | 'updated_at'>
    ...
    try {
      ...
      const result = db.runSync(
        ...
        [
          ...
          alert.location_coordinates
            ? JSON.stringify(alert.location_coordinates)
            : null,
          now,
        ]
      );
  ...
  async getPet(petId: string): Promise<any> {
    ...
    try {
      const result = db.getFirstSync('SELECT * FROM pets WHERE id = ?', [
        parseInt(petId),
      ]) as any;
  ...
  /**
   * Extract numeric weight from weight string (e.g., "25 kg" -> "25")
   */
  private extractWeightNumber(weight: string | null): number | undefined {
  ...
  private mapRowToPetProfile(row: any): StoredPetProfile {
    const emergencyContact = row.emergencyContact
      ? JSON.parse(row.emergencyContact)
      : undefined;

    return {
      id: row.id,
      user_id: row.user_id,
      ownerId: row.user_id, // Also add ownerId for compatibility
      ...
      // Map to snake_case field names that PetCard expects
      color_markings: row.colorMarkings,
      date_of_birth: row.dateOfBirth,
      weight_kg: this.extractWeightNumber(row.weight), // Extract numeric weight
      microchip_number: row.microchipId || row.registrationNumber, // Map microchip field
      special_notes: row.specialNotes,
      favorite_food: row.favoriteFood,
      exercise_needs: row.exerciseNeeds,
      personality_traits: row.personalityTraits
        ? JSON.parse(row.personalityTraits)
        : [],
      favorite_activities: row.favoriteActivities
        ? JSON.parse(row.favoriteActivities)
        : [],
      ...
      // Required fields for Pet interface
      createdAt: row.createdAt ? new Date(row.createdAt) : new Date(),
      updatedAt: row.updatedAt ? new Date(row.updatedAt) : new Date(),
      profileCompleteness: 0, // Default value, should be calculated

      // Status field for pet status badge
      status: 'active' as const, // Default status, can be 'active', 'lost', 'found', 'deceased'
    };
  }
}

...
export default function PetProfileScreen() {
  ...
  const loadPetData = useCallback(async () => {
    try {
      ...
      if (!vaccinationError) {
        // NOTE: Map database snake_case to VaccinationRecord camelCase interface
        const mapped = (vaccinationData || []).map(v => ({
          id: v.id,
          vaccine: v.vaccine_name || '',
          administeredDate: v.administered_date,
          nextDueDate: v.next_due_date || undefined,
          batchNumber: v.batch_number || undefined,
        })) as unknown as VaccinationRecord[]; // Double assertion needed for type conversion
      ...
      if (!medicalError) {
        ...
        setMedicalRecords((medicalData || []) as any);
      }
          ...
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                ...
                Alert.alert(
                  'Alert Sent',
                  'Nearby users have been notified about your lost pet.'
                );
              } catch (error) {
  ...
  const markFound = async () => {
    Alert.alert('Mark as Found', `Great news! Mark ${pet?.name} as found?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Mark Found',
        onPress: async () => {
          ...
          <View style={styles.insuranceCard}>
            ...
            <View style={styles.insuranceInfo}>
              ...
              {pet.insurance_contact_phone && (
                <Text style={styles.insuranceDetail}>
                  Contact: {pet.insurance_contact_phone}
                </Text>
              )}
            </View>
          </View>
      ...
      {pet?.breeding_status && pet.breeding_status !== 'not_applicable' && (
        <View style={styles.infoSection}>
          ...
          <View style={styles.textSection}>
            ...
            <Text style={styles.textValue}>
              {pet.breeding_status.charAt(0).toUpperCase() +
                pet.breeding_status.slice(1)}
            </Text>
          </View>
          ...
          {(pet.registration_number || pet.registration_organization) && (
            <View style={styles.textSection}>
              ...
              {pet.registration_organization && (
                <Text style={styles.textValue}>
                  Organization: {pet.registration_organization}
                </Text>
              )}
              {pet.registration_number && (
                <Text style={styles.textValue}>
                  Number: {pet.registration_number}
                </Text>
              )}
            </View>
          )}
            ...
            vaccinations.slice(0, 3).map((vaccination, index) => (
              <View key={vaccination.id} style={styles.healthRecord}>
                ...
                <View style={styles.healthRecordContent}>
                  <Text style={styles.healthRecordTitle}>
                  ...
                  <Text style={styles.healthRecordVet}>
      ...
      <SafeAreaView style={styles.qrModal}>
        ...
        <View style={styles.qrContent}>
          ...
          <Text style={styles.qrDescription}>
            Share this QR code with veterinarians or others who need access to{' '}
            {pet?.name}'s profile.
          </Text>
        </View>
      </SafeAreaView>

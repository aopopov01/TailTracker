...
export const SuccessCelebration: React.FC<CelebrationProps> = ({
  ...
  const startCelebration = useCallback(() => {
    ...
    titleOpacity.value = withDelay(
      400,
      withTiming(1, premiumAnimations.timings.standard)
    );
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    return {
      transform: [{ scale: containerScale.value }, { scale: pulseScale.value }],
    };
  });
...
const ConfettiAnimation: React.FC<ConfettiProps> = ({
  ...
  const renderConfettiPiece = (
    piece: (typeof confettiPieces.current)[0],
    index: number
  ) => {
    const animatedStyle = useAnimatedStyle(() => {
      const transforms = [];
      transforms.push({ translateX: piece.translateX.value });
      transforms.push({ translateY: piece.translateY.value });
      transforms.push({ rotate: `${piece.rotation.value}deg` });
      transforms.push({ scale: piece.scale.value });
      return {
        transform: transforms,
...
export const SuccessToast: React.FC<{
  ...
  return (
    <Animated.View
      style={[styles.toastContainer, getPositionStyle(), animatedStyle]}
    >

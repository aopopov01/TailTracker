...
interface OptimizedFlatListProps<T>
  extends Omit<FlatListProps<T>, 'getItemLayout' | 'keyExtractor'> {
...
function OptimizedFlatList<T>({
  ...
  const handleViewabilityChanged = useCallback(
  ...
  const optimizedRenderItem = useCallback<
    NonNullable<FlatListProps<T>['renderItem']>
  >(
    itemInfo => {
  ...
  const optimizedProps = useMemo(() => {
    const deviceOptimizations = {
      ...
      maxToRenderPerBatch:
        pixelRatio <= 2
          ? maxToRenderPerBatch
          : Math.max(maxToRenderPerBatch - 2, 5),
      initialNumToRender:
        pixelRatio <= 2
          ? initialNumToRender
          : Math.max(initialNumToRender - 2, 5),
  ...
  useEffect(() => {
    ...
    if (__DEV__) {
      console.log(
        '[OptimizedFlatList] Performance metrics:',
        performanceLogger
      );
    }
...
export const useFlatListPerformance = (listId: string) => {
  ...
  const trackRenderTime = useCallback(
    (startTime: number) => {
      ...
      if (__DEV__) {
        console.log(
          `[FlatList ${listId}] Render time: ${renderTime.toFixed(2)}ms`
        );
      }
    },
    [listId]

...
export class ApiClient {
  ...
  /**
   * Execute the actual request with simplified retry logic
   */
  private async executeRequest<T>(
    ...
    const maxDelayMs = config.retry?.maxDelayMs || 30000;
  ...
  public getCacheStats(): {
    ...
    const totalSizeBytes = JSON.stringify(
      Object.fromEntries(this.requestCache)
    ).length;
...
export const TailTrackerAPI = {
  ...
  vaccinations: {
    getByPetId: (petId: string) =>
      apiClient.get(`/vaccinations?pet_id=eq.${petId}`),
    create: (vaccinationData: any) =>
      apiClient.post('/vaccinations', vaccinationData),
    update: (id: string, vaccinationData: any) =>
      apiClient.put(`/vaccinations/${id}`, vaccinationData),
  ...
  lostPets: {
    ...
    markFound: (lostPetId: string, foundBy?: string) =>
      apiClient.post(
        '/functions/v1/lost-pet-alerts',
        {
          action: 'mark_found',
  ...
  // Image upload
  images: {
    upload: (imageData: any, type: string, petId?: string) =>
      apiClient.post(

...
const createEnvironmentConfig = (): EnvironmentConfig => {
  ...
  return {
    NODE_ENV: getEnvVar('NODE_ENV', false, 'development') as
      | 'development'
      | 'staging'
      | 'production',
    APP_ENV: currentEnv,
    ...
    ENABLE_PERFORMANCE_MONITORING: getBooleanEnvVar(
      'ENABLE_PERFORMANCE_MONITORING',
      isProduction
    ),

    // Security settings
    CERTIFICATE_PINNING_ENABLED: getBooleanEnvVar(
...
if (isDevelopment) {
  ...
  if (!validation.isValid) {
    ...
    console.warn(
      '   Set environment variables in your build system or .env file'
    );
  }
}

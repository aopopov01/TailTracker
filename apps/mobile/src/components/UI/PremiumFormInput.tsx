...
export const PremiumFormInput: React.FC<PremiumFormInputProps> = ({
  ...
  const animateToFocused = useCallback(() => {
    ...
    if (floatingLabel) {
      const labelAnimation = premiumAnimations.forms.floatingLabel(
        true,
        !!value
      );
  ...
  const animatedLabelStyle = useAnimatedStyle(() => {
    const transforms = [];
    transforms.push({ translateY: labelY.value });
    transforms.push({ scale: labelScale.value });

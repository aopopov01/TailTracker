...
export const PhotoUploadProgress: React.FC<PhotoUploadProps> = ({
  ...
  const handleSuccessState = useCallback(() => {
    ...
    successScale.value = withTiming(
      ...
      premiumAnimations.timings.standard
    );
  ...
  const handleErrorState = useCallback(() => {
    ...
    errorShake.value = withSequence(
      ...errorAnimation.translateX.map((x: number) =>
        withTiming(x, { duration: 50 })
      )
    );
  ...
  const animatedContainerStyle = useAnimatedStyle(() => {
    const transforms = [];
    if (containerScale.value !== 1) {
      transforms.push({ scale: containerScale.value });
    }
    if (errorShake.value !== 0) {
      transforms.push({ translateX: errorShake.value });
    }
    return {
    ...
    return (
      <View style={styles.errorOverlay}>
        ...
        {onRetry && (
          ...
          >
            <Text
              style={[styles.retryButtonText, { color: theme.colors.error }]}
            >
              Try Again
            </Text>
          </TouchableOpacity>
  ...
  const renderCancelButton = () => {
    ...
    return (
      <TouchableOpacity style={styles.topRightButton} onPress={onCancel}>

...
export const VaccinationCard: React.FC<VaccinationCardProps> = ({
  ...
  const isOverdue =
    vaccination.next_due_date && isDateInPast(vaccination.next_due_date);
  const isDueSoon =
    vaccination.next_due_date &&
    getDaysUntilDate(vaccination.next_due_date) <= 30 &&
    getDaysUntilDate(vaccination.next_due_date) > 0;
  ...
  const getStatusText = () => {
    ...
    if (isDueSoon)
      return `Due in ${getDaysUntilDate(vaccination.next_due_date)} days`;
    return 'Up to date';
  };
    ...
    >
      <View style={styles.header}>
        <View style={styles.titleContainer}>
          ...
          <Text style={styles.date}>
            Administered: {formatDate(vaccination.administered_date)}
          </Text>
          {vaccination.next_due_date && (
      ...
      {vaccination.veterinarian && (
        <Text style={styles.veterinarian}>
          ...
          {vaccination.veterinarian.clinic_name &&
            ` - ${vaccination.veterinarian.clinic_name}`}
        </Text>
      )}
      ...
      {vaccination.certificate_url && (
        <View style={styles.certificateContainer}>
          <Image
            source={{ uri: vaccination.certificate_url }}

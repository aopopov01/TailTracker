...
export const RegisterScreen: React.FC<RegisterScreenProps> = ({
  ...
  useEffect(() => {
    const validatePassword = async () => {
      if (formData.password) {
        const validation = await CryptoService.validatePasswordStrength(
          formData.password
        );
  ...
  const renderPasswordStrength = () => {
    ...
    const strength = passwordValidation.isValid ? 'Strong' : 'Weak';
    const strengthColor = passwordValidation.isValid ? '#28a745' : '#dc3545';
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            Join TailTracker to manage your pet profiles
          </Text>
        </View>

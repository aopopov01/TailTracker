...
describe('useLostPetNotifications', () => {
  ...
  // Helper to render hook and wait for initial async effects
  const renderNotificationHook = async () => {
    const hookResult = renderHook(() => useLostPetNotifications());
  ...
  beforeEach(() => {
    ...
    mockNotificationService.getPushToken.mockResolvedValue(
    ...
    mockNotificationService.getCurrentPushToken.mockReturnValue(
      'ExponentPushToken[test]'
    );
  ...
  describe('initialization', () => {
    ...
    it('should initialize notifications on mount', async () => {
      ...
      expect(mockNotificationService.updateUserPushToken).toHaveBeenCalledWith(
        'user-1'
      );
    });

    it('should handle initialization error', async () => {
      require('../../services/NotificationService').NotificationHelpers.areNotificationsEnabled.mockRejectedValue(
  ...
  describe('requestPermissions', () => {
    it('should request and grant permissions successfully', async () => {
      const { result } = await renderNotificationHook();
  ...
  describe('disableNotifications', () => {
    it('should disable notifications successfully', async () => {
      ...
      expect(
        mockNotificationService.disablePushNotifications

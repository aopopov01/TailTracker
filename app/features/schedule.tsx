...
export default function ScheduleScreen() {
  ...
  const loadEvents = async () => {
    try {
      ...
      // Note: Database integration to be implemented in future iteration
      setEvents([]);
    } catch (error) {
      setEvents([]);
    }
  ...
  const syncToPhoneCalendar = async (event: ScheduleEvent) => {
    try {
      ...
      setEvents(prev =>
        prev.map(e => (e.id === event.id ? { ...e, calendarEventId } : e))
      );
  ...
  const upcomingEvents = events.filter(
    event => !event.completed && event.date >= new Date()
  );
              ...
              >
                <View style={styles.eventHeader}>
                  <View style={styles.eventTypeIndicator}>
                    ...
                    <View style={styles.eventInfo}>
                      <Text
                        style={[styles.eventTitle, styles.completedEventTitle]}
                      >
                        {event.title}
                      </Text>
                      <Text style={styles.eventDateTime}>
                ...
                >
                  {Object.entries(eventTypeConfig).map(([type, config]) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setNewEvent(prev => ({ ...prev, type: type as any }))
                      }
                    >
                      <View
                        style={[

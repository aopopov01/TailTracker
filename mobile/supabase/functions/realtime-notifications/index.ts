...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { type, recipients, payload, region }: BroadcastNotification = await req.json();

      let targetUsers = recipients;

      // If region is specified, find users in that area
      if (region) {
        const { data: nearbyUsers, error } = await supabaseClient
      ...
      const eligibleUsers = userTokens?.filter(user => {

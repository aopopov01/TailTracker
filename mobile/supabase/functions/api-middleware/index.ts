...
interface BusinessRule {
  name: string;
  description: string;
  validate: (data: any, context: any) => Promise<{ valid: boolean; error?: string }>;
}
...
const petValidationRules: ValidationRule[] = [
  ...
  { field: 'species', type: 'string', required: true, enum: ['Dog', 'Cat', 'Bird', 'Rabbit', 'Guinea Pig', 'Hamster', 'Fish', 'Reptile', 'Other'] },
  ...
  { field: 'microchip_id', type: 'string', maxLength: 50, pattern: '^[A-Z0-9]{15}$' },
];

// User profile validation rules
const userValidationRules: ValidationRule[] = [
  { field: 'email', type: 'email', required: true },
  { field: 'full_name', type: 'string', required: true, minLength: 2, maxLength: 255 },
  { field: 'subscription_tier', type: 'string', enum: ['free', 'premium', 'family'] },
];

// Vaccination validation rules
const vaccinationValidationRules: ValidationRule[] = [
  { field: 'pet_id', type: 'uuid', required: true },
  { field: 'vaccine_name', type: 'string', required: true, minLength: 1, maxLength: 100 },
...
const businessRules: BusinessRule[] = [
  {
    ...
    description: 'Free users can have max 3 pets, premium users can have unlimited',
    validate: async (data: any, context: any) => {
      ...
      if (userProfile?.subscription_tier === 'free') {
        ...
        if (count >= 3) {
          return {
            valid: false,
            error: 'Free accounts are limited to 3 pets. Upgrade to premium for unlimited pets.',
          };
        }
      }
  ...
  },
  {
    ...
    description: 'Vaccination dates must be logical (administered <= today, next due > administered)',
    validate: async (data: any, context: any) => {
      ...
      const nextDueDate = data.next_due_date ? new Date(data.next_due_date) : null;
...
const validateField = (value: any, rule: ValidationRule): { valid: boolean; error?: string } => {
  // Check required
  ...
  switch (rule.type) {
    case 'string':
      ...
      if (rule.enum && !rule.enum.includes(value)) {
        return { valid: false, error: `${rule.field} must be one of: ${rule.enum.join(', ')}` };
      }
      break;
    ...
    case 'email':
      ...
      if (!emailRegex.test(value)) {
        return { valid: false, error: `${rule.field} must be a valid email address` };
      }
      break;
...
const validateData = (data: any, rules: ValidationRule[]): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const rule of businessRules) {
      const result = await rule.validate({ table, operation, ...data }, context);
      if (!result.valid) {

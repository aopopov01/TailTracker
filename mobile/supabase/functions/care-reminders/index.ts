...
interface CareReminder {
  ...
  pet_id: string;
  user_id: string;
  reminder_type: 'vaccination' | 'medication' | 'grooming' | 'vet_checkup' | 'custom';
  title: string;
  description: string;
  due_date: string;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { reminders, advance_notice_hours = 24 }: ScheduleRemindersRequest = await req.json();
      ...
      const scheduledNotifications = await Promise.all(
        reminders.map(async (reminder) => {
          ...
          const notificationDate = new Date(dueDate.getTime() - (advance_notice_hours * 60 * 60 * 1000));

...
interface VeterinarianWebhook {
  event_type: 'appointment_scheduled' | 'vaccination_completed' | 'medical_record_updated';
  clinic_id: string;
  pet_id: string;
  owner_id: string;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const webhookSecret = Deno.env.get(`WEBHOOK_SECRET_${webhookType?.toUpperCase()}`);
    ...
    switch (webhookType) {
      case 'veterinarian':
        return await handleVeterinarianWebhook(supabaseClient, payload as VeterinarianWebhook);
      
      case 'insurance':
        return await handleInsuranceWebhook(supabaseClient, payload as InsuranceWebhook);
      
      case 'microchip':
        return await handleMicrochipWebhook(supabaseClient, payload as MicrochipWebhook);
...
async function handleVeterinarianWebhook(
  ...
  switch (payload.event_type) {
    case 'vaccination_completed':
      ...
      if (vaccinationError) {
        throw new Error(`Failed to record vaccination: ${vaccinationError.message}`);
      }
...
async function handleInsuranceWebhook(
  ...
  const notificationTitle = payload.event_type === 'claim_approved' 
    ? 'Insurance Claim Approved' 
    : payload.event_type === 'claim_denied'
    ? 'Insurance Claim Update'
    : 'Policy Update';

  const notificationBody = payload.event_type === 'claim_approved'
  ...
  switch (payload.event_type) {
    case 'pet_found':
        ...
        await supabaseClient
          ...
          .update({
            ...
            last_seen_location: payload.found_location ? 
              `POINT(${payload.found_location.longitude} ${payload.found_location.latitude})` : null,
          })
          .eq('id', pet.id);
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: pet.status === 'lost' ? 'Pet found and owner notified' : 'Pet scanned successfully',
          pet_info: {
            name: pet.name,
            status: pet.status,
          },

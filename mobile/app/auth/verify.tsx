  ...
  useEffect(() => {
    const handleEmailVerification = async () => {
      try {
        ...
        if (!accessToken) {
          accessToken = params['access-token'] as string || params.token as string;
        }
        if (!refreshToken) {
        ...
        const confirmationToken = params.token as string || params.confirmation_token as string;
        ...
        if (confirmationToken && !accessToken) {
          ...
          if (data.user && data.user.email_confirmed_at) {
            console.log('✅ Email verification successful via confirmation token');
        ...
        if (!accessToken || !refreshToken) {
          console.error('Missing verification tokens - all params:', Object.keys(params));
          modalService.showError(
            'Verification Failed',

  ...
  const toggleSafeZone = (petId: string) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Set Zone', onPress: () => console.log('Setting safe zone for pet:', petId) }
      ]
    );
  };
        ...
        {pets.map((pet) => (
          <React.Fragment key={pet.id}>
            {pet.location && (
              ...
              >
                <View style={[styles.petMarker, { backgroundColor: pet.color || theme.colors.primary }]}>
      ...
      <View style={[styles.petList, { backgroundColor: theme.colors.surface }]}>
        {pets.map((pet) => (
          <TouchableOpacity
            ...
            style={[
              styles.petItem,
              selectedPet?.id === pet.id && { backgroundColor: theme.colors.primaryContainer }
            ]}
            onPress={() => handlePetSelect(pet)}
            onLongPress={() => toggleSafeZone(pet.id)}
          >
            <View style={[styles.petIndicator, { backgroundColor: pet.color || theme.colors.primary }]} />
            <Text style={[styles.petName, { color: theme.colors.onSurface }]}>
              {pet.name}
            </Text>
            {pet.location && (
              <Text style={[styles.petStatus, { color: theme.colors.onSurfaceVariant }]}>
                Active
              </Text>
            )}
          </TouchableOpacity>
            ...
            {selectedPet.location && (
              <>
                ...
                <View style={styles.infoRow}>
                  ...
                  <Text style={[styles.infoText, { color: theme.colors.onSurfaceVariant }]}>
                    Location: {selectedPet.location.latitude.toFixed(4)}, {selectedPet.location.longitude.toFixed(4)}
                  </Text>
                </View>
              </>
            )}

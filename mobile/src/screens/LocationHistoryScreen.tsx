...
type LocationHistoryScreenRouteProp = RouteProp<RootStackParamList, 'LocationHistory'>;
type LocationHistoryScreenNavigationProp = StackNavigationProp<RootStackParamList, 'LocationHistory'>;
...
export const LocationHistoryScreen: React.FC<Props> = ({ route, navigation }) => {
      ...
      <Card style={[styles.sessionCard, { backgroundColor: theme.colors.surface }]}>
        <Card.Content>
          ...
          <View style={styles.sessionDetails}>
            ...
            <View style={styles.detailItem}>
              <Icon name="map-marker-multiple" size={16} color={theme.colors.onSurfaceVariant} />
              <Text style={[styles.detailText, { color: theme.colors.onSurfaceVariant }]}>
          ...
          <View style={styles.activityContainer}>
            {[...new Set(item.locations.map(loc => loc.activity).filter(Boolean))].map((activity, index) => (
              <Chip
                key={index}
                icon={() => <Icon name={getActivityIcon(activity)} size={14} color={getActivityColor(activity)} />}
                style={[styles.activityChip, { backgroundColor: `${getActivityColor(activity)}20` }]}
    ...
    if (!selectedSession || selectedSession.locations.length === 0) {
      return (
        <View style={styles.emptyMapContainer}>
          <Icon name="map-outline" size={64} color={theme.colors.onSurfaceVariant} />
          <Text style={[styles.emptyMapText, { color: theme.colors.onSurfaceVariant }]}>
    ...
    return (
      <View style={styles.mapContainer}>
        ...
        >
          ...
          >
            <View style={[styles.startMarker, { backgroundColor: theme.colors.tertiary }]}>
          ...
          >
            <View style={[styles.endMarker, { backgroundColor: theme.colors.error }]}>
          ...
          {selectedSession.locations
            .filter((loc, index) => index > 0 && index < selectedSession.locations.length - 1 && loc.activity)
            .map((location) => (
              <Marker
                key={location.id}
              ...
              >
                <View style={[styles.activityMarker, { backgroundColor: getActivityColor(location.activity) }]}>
                  <Icon name={getActivityIcon(location.activity)} size={12} color="white" />
                </View>
              </Marker>
            ))
          }
        ...
        {/* Map session info */}
        <Card style={[styles.mapInfoCard, { backgroundColor: theme.colors.surface }]}>
          <Card.Content style={styles.mapInfoContent}>
            <View style={styles.mapInfoItem}>
              <Text style={[styles.mapInfoValue, { color: theme.colors.primary }]}>
                {formatDistance(selectedSession.distance)}
              </Text>
              <Text style={[styles.mapInfoLabel, { color: theme.colors.onSurfaceVariant }]}>
                Distance
              </Text>
            </View>
            <View style={styles.mapInfoItem}>
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.surface }]}>
        ...
        <View style={styles.controlsRow}>
          ...
          <View style={styles.filterContainer}>
            {(['today', '3days', '7days', '30days', 'all'] as TimeFilter[]).map((filter) => (
              <Chip
                key={filter}
          ...
          <View style={styles.viewToggle}>
            ...
            <TouchableOpacity
              style={[
                styles.viewButton,
                viewMode === 'map' && { backgroundColor: theme.colors.primaryContainer }
              ]}
            ...
            >
              <Icon 
                name="map" 
                size={20} 
                color={viewMode === 'map' ? theme.colors.onPrimaryContainer : theme.colors.onSurfaceVariant} 
              />
            </TouchableOpacity>
          </View>
        </View>

...
export default function LostPetAlertScreen() {
  const { petId, alertId } = useLocalSearchParams<{ petId: string; alertId?: string }>();
  ...
  const getCurrentLocation = useCallback(async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission Required', 'Location permission is needed to set the lost location');
        return;
      }
      ...
      if (!alertData.lost_location) {
        ...
        if (address && address.length > 0) {
          ...
          const locationString = `${addr.street || ''} ${addr.city || ''}, ${addr.region || ''} ${addr.postalCode || ''}`.trim();
          setAlertData(prev => ({
            ...prev,
  ...
  const removeEmergencyContact = (index: number) => {
    if (alertData.emergency_contacts.length > 1) {
      const newContacts = alertData.emergency_contacts.filter((_, i) => i !== index);
      setAlertData(prev => ({ ...prev, emergency_contacts: newContacts }));
    }
  };
  ...
  const createAlert = async () => {
    if (!alertData.lost_location || !alertData.description) {
      Alert.alert('Required Fields', 'Please fill in the lost location and description');
      return;
    }

    const validContacts = alertData.emergency_contacts.filter(contact => contact.trim() !== '');
...
Emergency Contacts:
${alertData.emergency_contacts.filter(c => c.trim()).map(contact => `• ${contact}`).join('\n')}
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {pet && (
          <View style={styles.petCard}>
            <View style={styles.petHeader}>
              ...
              <View style={[
                styles.statusBadge, 
                activeAlert?.is_active ? styles.activeBadge : styles.inactiveBadge
              ]}>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              ...
              onChangeText={(text) => updateField('reward_amount', parseInt(text) || 0)}
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            ...
            • QR codes on your pet's profile can help finders contact you{'\n'}
            • Update the alert with new sightings or information{'\n'}
            • Deactivate when your pet is found safely
          </Text>
        </View>

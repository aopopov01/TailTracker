...
const RECORD_TYPE_INFO = {
  ...
  prescription: { label: 'Prescription', icon: 'medical', color: colors.primary },
  test_result: { label: 'Test Result', icon: 'document-text', color: colors.info },
...
export default function MedicalRecordsScreen() {
      ...
      >
        ...
        {Object.entries(RECORD_TYPE_INFO).map(([type, info]) => {
          ...
          return (
            <TouchableOpacity
              key={type}
              style={[styles.filterTab, selectedType === type && styles.activeFilterTab]}
            ...
            >
              ...
              />
              <Text style={[styles.filterTabText, selectedType === type && styles.activeFilterTabText]}>
                {info.label} ({count})
              </Text>
            </TouchableOpacity>
          );
  ...
  const renderRecordItem = ({ item }: { item: MedicalRecord }) => {
    const typeInfo = RECORD_TYPE_INFO[item.type];

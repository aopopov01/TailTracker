 ...
 */
export function categorizeImport(importStatement: string): keyof typeof IMPORT_ORDER | 'UNKNOWN' {
 ...
 */
export function sortImportsInCategory(imports: string[], category: keyof typeof IMPORT_ORDER): string[] {
...
export function validateImportOrganization(fileContent: string): {
  ...
  const issues: {line: number; issue: string; suggestion: string; severity: 'error' | 'warning' | 'info'}[] = [];

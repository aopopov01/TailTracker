...
class PerformanceOptimizerService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.performanceSettings = this.generateOptimalSettings(this.deviceCapabilities);
  ...
  private async detectDeviceCapabilities(): Promise<DeviceCapabilities> {
    ...
    try {
      ...
      if (Platform.OS === 'ios') {
        ...
        if (screenSize > 1800000 || isTablet) { // iPad or iPhone Pro
          memoryClass = 'high';
        } else if (screenSize > 1000000) { // Standard iPhone
          memoryClass = 'medium';
        } else { // Older devices
          memoryClass = 'low';
        }
   ...
   */
  private generateOptimalSettings(capabilities: DeviceCapabilities): PerformanceSettings {
  ...
  adaptSettings(): void {
    ...
    if (avgRenderTime > 16) { // 60fps threshold
      ...
      this.performanceSettings.imageQuality = Math.max(0.5, this.performanceSettings.imageQuality - 0.1);
    }

    // If memory usage is high, reduce cache size
    if (avgMemoryUsage > 150 * 1024 * 1024) { // 150MB threshold
      this.performanceSettings.cacheSize = Math.max(
        10 * 1024 * 1024,
        this.performanceSettings.cacheSize * 0.8
      );
      this.performanceSettings.maxConcurrentImages = Math.max(3, this.performanceSettings.maxConcurrentImages - 2);
    }
  }
  ...
  async loadSettings(): Promise<void> {
    try {
      const stored = await AsyncStorage.getItem('@TailTracker:performance_settings');
      if (stored) {

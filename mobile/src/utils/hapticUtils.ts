...
const createHeartbeatPattern = async (intensity: HapticIntensity) => {
  const impactType = intensity === 'subtle' 
...
const createPlayfulPattern = async (intensity: HapticIntensity) => {
...
const createGentlePattern = async (intensity: HapticIntensity) => {
  // Soft, caring pattern
  await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
...
const createUrgentPattern = async (intensity: HapticIntensity) => {
...
export const buttonPressHaptic = (variant: 'primary' | 'secondary' | 'tertiary' = 'primary') => {
  switch (variant) {
    case 'primary':
...
export const petMoodHaptic = (mood: 'happy' | 'playful' | 'sleepy' | 'excited') => {
  switch (mood) {
    case 'happy':

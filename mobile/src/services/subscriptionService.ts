...
export const SUBSCRIPTION_PLANS: Record<SubscriptionTier, SubscriptionPlan> = {
  ...
  premium: {
    ...
    price: {
      ...
      yearly: 50.00, // €50/year from app store description
    },
    features: {
  ...
  pro: {
    ...
    price: {
      ...
      yearly: 80.00, // €80/year from app store description
    },
    features: {
...
class SubscriptionService {
   ...
   */
  async canAddFamilyMember(userId: string, currentFamilyCount: number): Promise<boolean> {
    const features = await this.getSubscriptionFeatures(userId);
   ...
   */
  async canAddPhoto(userId: string, currentPhotoCount: number): Promise<boolean> {
    const features = await this.getSubscriptionFeatures(userId);
  ...
  async isOnTrial(userId: string): Promise<boolean> {
    ...
    if (subscription.subscription_expires_at) {
      const now = new Date();
      const daysRemaining = (subscription.subscription_expires_at.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private mapStatusToTier(status: string): SubscriptionTier {
    // For now, we need to determine the tier from the status
    // In the future, we might want to add a separate tier column to user_profiles
    // For now, 'active' users are treated as 'premium', 'inactive' as 'free'
    switch (status) {
      case 'active':
  ...
  private async cacheSubscription(subscription: UserSubscription | null): Promise<void> {
    try {

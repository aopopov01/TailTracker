...
export const withAutoPopulate = <P extends object>(
  ...
  const WithAutoPopulateComponent = forwardRef<any, P & WithAutoPopulateProps>((props, ref) => {
    ...
    const enhancedProps = {
      ...
      getAutoValue: (fieldName: string, context?: 'pet' | 'user' | 'medical') => {
        const ctx = context || config.context;
        const contextMappings = ctx ? FIELD_MAPPINGS[ctx] : {};
        const mappedField = config.fieldMappings?.[fieldName] || contextMappings?.[fieldName] || fieldName;
        return dataSync.getFieldValue(mappedField, ctx);
      },
      
      setAutoValue: (fieldName: string, value: any, context?: 'pet' | 'user' | 'medical') => {
        const ctx = context || config.context;
        const contextMappings = ctx ? FIELD_MAPPINGS[ctx] : {};
        const mappedField = config.fieldMappings?.[fieldName] || contextMappings?.[fieldName] || fieldName;
        dataSync.setFieldValue(mappedField, value, ctx);
      },
      ...
      autoPopulatedValues: config.enableAutoPopulate ? (() => {
        ...
        const mappings = config.fieldMappings || FIELD_MAPPINGS[ctx] || {};
        
        Object.entries(mappings).forEach(([formField, dataField]) => {
          const value = dataSync.getFieldValue(dataField, ctx);
      ...
      getAutoValue: (fieldName: string, context?: 'pet' | 'user' | 'medical') => any;
      setAutoValue: (fieldName: string, value: any, context?: 'pet' | 'user' | 'medical') => void;
...
export const useFormAutoPopulate = (config: AutoPopulateConfig = {}) => {
  ...
  const getFieldValue = (fieldName: string, context?: 'pet' | 'user' | 'medical') => {
    const ctx = context || finalConfig.context;
    const contextMappings = ctx ? FIELD_MAPPINGS[ctx] : {};
    const mappedField = finalConfig.fieldMappings?.[fieldName] || contextMappings?.[fieldName] || fieldName;
    return dataSync.getFieldValue(mappedField, ctx);
  };

  const setFieldValue = (fieldName: string, value: any, context?: 'pet' | 'user' | 'medical') => {
    const ctx = context || finalConfig.context;
    const contextMappings = ctx ? FIELD_MAPPINGS[ctx] : {};
    const mappedField = finalConfig.fieldMappings?.[fieldName] || contextMappings?.[fieldName] || fieldName;
    dataSync.setFieldValue(mappedField, value, ctx);

...
interface AutoPopulateFieldProps extends Omit<TextInputProps, 'value' | 'onChangeText'> {
  fieldPath: string;
...
export const AutoPopulateField: React.FC<AutoPopulateFieldProps> = ({
  ...
  useEffect(() => {
    if (enableAutoPopulate && !hasBeenManuallyEdited && hasAutoValue && !currentValue) {
      setLocalValue(autoValue);
    }
  }, [autoValue, hasAutoValue, enableAutoPopulate, hasBeenManuallyEdited, currentValue]);

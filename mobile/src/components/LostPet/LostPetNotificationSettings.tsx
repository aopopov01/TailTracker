...
export const LostPetNotificationSettings: React.FC<LostPetNotificationSettingsProps> = ({
  showTitle = true,
}) => {
  ...
  const handleClearNotifications = () => {
    if (pendingNotifications === 0) {
      Alert.alert('No Notifications', 'You have no pending lost pet alerts to clear.');
      return;
    }

    Alert.alert(
      'Clear Notifications?',
      `This will clear ${pendingNotifications} pending lost pet alert${pendingNotifications > 1 ? 's' : ''}.`,
      [
        { text: 'Cancel', style: 'cancel' },
    ...
    <Card style={styles.card}>
      <Card.Content>
        ...
        <Text style={styles.description}>
          Get notified when pets go missing in your area so you can help reunite them with their families.
        </Text>

        {!hasPremiumAccess && (
          <View style={styles.premiumNotice}>
            <Text style={styles.premiumText}>
              🌟 Lost pet alerts are available for all users. Premium users can also report their own pets as lost and send regional alerts.
            </Text>
          </View>
        )}
            ...
            {pendingNotifications > 0 && (
              <View style={styles.pendingRow}>
                <Text style={styles.pendingText}>
                  📱 {pendingNotifications} pending alert{pendingNotifications > 1 ? 's' : ''}
                </Text>
                <Button
                  mode="outlined"
                  compact

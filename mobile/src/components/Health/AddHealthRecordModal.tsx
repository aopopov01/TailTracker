...
export const AddHealthRecordModal: React.FC<AddHealthRecordModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Upload photos if any (only for new records or if editing and new photos added)
      if (selectedPhotos.length > 0 && result.healthRecord) {
        for (const photoUri of selectedPhotos) {
          const uploadResult = await healthRecordsService.uploadHealthRecordPhoto(
            result.healthRecord.id,
            photoUri
          );
          
          if (!uploadResult.success) {
            Alert.alert('Photo Upload Error', uploadResult.error || 'Failed to upload photo');
      ...
      onSuccess();
      onClose();
      Alert.alert('Success', `Health record ${record ? 'updated' : 'created'} successfully`);

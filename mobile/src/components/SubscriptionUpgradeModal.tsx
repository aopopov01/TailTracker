...
import { SubscriptionTier, SUBSCRIPTION_PLANS, SubscriptionPlan } from '../services/subscriptionService';
...
const PlanCard: React.FC<PlanCardProps> = ({
  ...
  const getFeatureLabel = (feature: string, value: any): string => {
    switch (feature) {
      case 'maxPets':
        return value === 999 ? 'Unlimited pets' : `${value} pet profile${value > 1 ? 's' : ''}`;
      case 'maxFamilyMembers':
        return value === 999 ? 'Unlimited family members' : `${value} family members total`;
      case 'photosPerPet':
      ...
      default:
        return feature.replace(/([A-Z])/g, ' $1').toLowerCase().replace(/^./, str => str.toUpperCase());
    }
  };
    ...
    >
      ...
      >
        <View style={styles.planHeader}>
          ...
          <Text style={[styles.planDescription, { color: getTextColor(), opacity: 0.8 }]}>
            {plan.description}
          </Text>
        </View>
        ...
        <View style={styles.priceContainer}>
          {plan.price.monthly === 0 ? (
            ...
            <>
              ...
              <Text style={[styles.yearlyPrice, { color: getTextColor(), opacity: 0.7 }]}>
                or €{plan.price.yearly}/year (save {Math.round(((plan.price.monthly * 12 - plan.price.yearly) / (plan.price.monthly * 12)) * 100)}%)
              </Text>
            </>
          )}
        </View>
        ...
        <View style={styles.featuresContainer}>
          {Object.entries(plan.features).map(([feature, value]) => {
            if (typeof value === 'boolean' && !value && plan.id !== 'free') return null;
            ...
            >
              ...
              >
                <Text style={styles.upgradeButtonText}>
                  {isUpgrading ? 'Processing...' : `Upgrade to ${SUBSCRIPTION_PLANS[selectedTier].name}`}
                </Text>
                {!isUpgrading && (
